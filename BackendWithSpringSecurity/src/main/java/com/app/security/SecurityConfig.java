...
public class SecurityConfig {
	...
	public SecurityFilterChain authorizeRequests(HttpSecurity http) throws Exception {
		...
		// disable CSRF token generation n verification
				...
				.antMatchers("/api/citizen/signin", "/api/center/signin", "/api/admin/signin", "/api/center/centers/{pinCode}","/api/citizen/statistics","/api/center/find/{id}","/api/citizen/register").permitAll()
				.antMatchers("/api/citizen").hasRole("CITIZEN").antMatchers("/api/center").hasRole("CENTER").antMatchers("/api/admin").hasRole("ADMIN").anyRequest().authenticated()
				.and().httpBasic().and()
